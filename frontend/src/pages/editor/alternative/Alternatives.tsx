...
import CategoryTable, {
	type Subcategories,
} from "pages/editor/alternative/CategoryTable";
...
const energyCategories$ = AlternativeModel.energyCosts$.pipe(
	map((costs) =>
		group(
			...
			(cost) => (cost as EnergyCost).fuelType ?? FuelType.OTHER,
		),
	),
);
...
const waterCosts$ = AlternativeModel.waterCosts$.pipe(
	map(
		(costs) =>
			({ [CostTypes.WATER]: costs }) as unknown as Subcategories<CostTypes>,
	),
);
...
const capitalCategories$ = AlternativeModel.capitalCosts$.pipe(
	map((costs) =>
		group(
			costs,
			[CostTypes.CAPITAL, CostTypes.REPLACEMENT_CAPITAL, CostTypes.OMR],
			(cost) => cost.type,
		),
	),
);
...
const contractCategories$ = AlternativeModel.contractCosts$.pipe(
	map((costs) =>
		group(
			costs,
			[CostTypes.IMPLEMENTATION_CONTRACT, CostTypes.RECURRING_CONTRACT],
			(cost) => cost.type,
		),
	),
);
...
const otherCategories$ = AlternativeModel.otherCosts$.pipe(
	map((costs) =>
		group(
			costs,
			[CostTypes.OTHER, CostTypes.OTHER_NON_MONETARY],
			(cost) => cost.type,
		),
	),
);
...
export default function Alternatives() {
	...
	const [
		...
		const baselineChangeNoConfirm$ = merge(
			disableBaseline$,
			enableBaselineWithoutModal$,
		);
	...
	useSubscribe(
		merge(confirmBaselineChange$, baselineChangeNoConfirm$),
		AlternativeModel.sBaseline$,
	);
			...
			<div className={"flex flex-col p-6 h-full overflow-y-auto"}>
				...
				<div className={"flex justify-between border-b-2 border-base-lightest"}>
					...
					<Button
						...
						onClick={() => openCostModal$.next(CostTypes.CAPITAL)}
					>
