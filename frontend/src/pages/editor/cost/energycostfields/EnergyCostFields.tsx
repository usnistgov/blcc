...
export default function EnergyCostFields() {
	const [
		...
		const collection$ = CostModel.collection$ as Observable<
			Collection<EnergyCost, number>
		>;

		const sCostPerUnit$ = new Subject<number>();
		const costPerUnit$ = merge(
			sCostPerUnit$,
			EnergyCostModel.cost$.pipe(map((cost) => cost.costPerUnit)),
		...
		const rebate$ = merge(
			sRebate$,
			EnergyCostModel.cost$.pipe(map((cost) => cost.rebate)),
		).pipe(distinctUntilChanged());

		const sDemandCharge$ = new Subject<number | undefined>();
		const demandCharge = merge(
			sDemandCharge$,
			EnergyCostModel.cost$.pipe(map((cost) => cost.demandCharge)),
		).pipe(distinctUntilChanged());
	...
	const customLocation = useStateObservable(
		EnergyCostModel.Location.customLocation$,
	);
