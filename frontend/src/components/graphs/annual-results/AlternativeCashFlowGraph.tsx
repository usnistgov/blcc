...
type AlternativeNpvCashFlowGraphColumns =
...
function getColumns(
	...
	for (const category of ResultModel.categories) {
		columns = [
			getColumn(altId, optionals, category, discountedCashFlow),
			...columns,
		];
	}
	return columns;
...
export default function AlternativeCashFlowGraph({
	...
	const graphId = offscreen
		? `${OFFSCREEN_GRAPH_ID_TEMPLATE}-${selection}`
		: GRAPH_ID;
	...
	}, [chart, selection, optionals, discountedCashFlow]);
