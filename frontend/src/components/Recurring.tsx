...
import {
	type RateChangeInfo,
	RecurringModel,
} from "model/costs/RecurringModel";
...
function renderCell(info: RenderCellProps<RateChangeInfo>) {
	...
	const rate = isDollarMethodCurrent
		? calculateNominalDiscountRate(info.row.rate, inflation)
		: info.row.rate;
...
function renderEditCell({
	...
	column,
	onRowChange,
}: RenderEditCellProps<RateChangeInfo>) {
	const isDollarMethodCurrent = Model.useIsDollarMethodCurrent();
	const inflation = Model.inflationRate.use() ?? Defaults.INFLATION_RATE;

	const rate = isDollarMethodCurrent
		? calculateNominalDiscountRate(row.rate, inflation)
		: row.rate;
...
export function RateOfRecurrenceInput({
	showLabel = false,
}: RateOfRecurrenceInputProps) {
	return (
		<TestNumberInput
			className={"w-full"}
...
function ValueRateOfChangeGrid() {
	const columns = useMemo(
		() => createDataGridColumns("Value Rate of Change"),
		[],
	);
	...
	return (
		<div>
			<TestNumberInput
				...
				onChange={(v) =>
					RecurringModel.Value.Actions.setConstant(
						v,
						inflationRate,
						isDollarMethodCurrent,
					)
...
function UnitRateOfChangeGrid() {
	const columns = useMemo(
		() => createDataGridColumns("Unit Rate of Change"),
		[],
	);
