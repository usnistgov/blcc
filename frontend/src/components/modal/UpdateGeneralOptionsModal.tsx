...
export function showUpdateGeneralOptionsModal(): OperatorFunction<
	unknown,
	void
> {
	return pipe(
...
export namespace UpdateGeneralOptionsModel {
	...
	export const [useOpen] = bind(
		merge(
			merge(cancel$, update$).pipe(map(() => false)),
			open$.pipe(map(() => true)),
		),
		false,
	);
			...
			footer={
				<div className={"mt-8 flex w-full flex-row justify-end gap-4"}>
					<Button
						...
						icon={mdiClose}
						onClick={UpdateGeneralOptionsModel.cancel}
					>
						Don't Update
					</Button>
					<Button
						...
						icon={mdiCheck}
						onClick={UpdateGeneralOptionsModel.update}
					>
						Update
					</Button>
				</div>
		...
		>
			<div className={"mt-8 flex flex-col items-center gap-4"}>
				<p className={"w-80 text-justify"}>
					Changing the <b>Analysis Type</b>, <b>Data Release Year</b>, or the{" "}
					<b>Project Purpose</b> may also update the <b>Dollar Analysis</b>,{" "}
					<b>Discounting Convention</b>, and <b>Discount Rates</b>.
				</p>
				<p className={"w-80 text-justify"}>
					Do you wish to keep those values the same or update to default?
				</p>
			</div>
		</Modal>
	);
...
function setOmbNonEnergy(
	...
	if (purpose === Purpose.INVEST_REGULATION) {
		...
		Model.realDiscountRate.set(real);
		Model.nominalDiscountRate.set(
			calculateNominalDiscountRate(real, rate.inflation),
		);
	} else {
		Model.realDiscountRate.set(rate.real);
		Model.nominalDiscountRate.set(
			calculateNominalDiscountRate(rate.real, rate.inflation),
		);
	}
}
	...
	BlccRuntime.runPromise(
		Effect.gen(function* () {
			...
			const [doeDiscountRates, ombDiscountRates] = yield* Effect.all(
				[
					api.fetchDoeDiscountRates(releaseYear),
					api.fetchOmbDiscountRates(releaseYear),
				],
				{ concurrency: 2 },
			);
			...
			Match.value(analysisType).pipe(
				Match.when(AnalysisType.FEDERAL_FINANCED, () =>
					setFederalFinanced(doeDiscountRates),
				),
				Match.when(AnalysisType.FEMP_ENERGY, () =>
					setFempEnergy(doeDiscountRates),
				),
				Match.when(AnalysisType.OMB_NON_ENERGY, () =>
					setOmbNonEnergy(ombDiscountRates, purpose, studyPeriod),
				),
				Match.when(AnalysisType.MILCON_NON_ENERGY, () =>
					setMilconNonEnergy(ombDiscountRates, studyPeriod),
				),
