...
const projectCollection$ = currentProject$.pipe(
	map((id) => db.projects.where("id").equals(id)),
);

const dbProject$ = currentProject$.pipe(
	switchMap((currentID) =>
		liveQuery(() => db.projects.where("id").equals(currentID).first()),
	),
	guard(),
);
...
export const alternatives$ = alternativeIDs$.pipe(
	switchMap((ids) =>
		liveQuery(() => db.alternatives.where("id").anyOf(ids).toArray()),
	),
	shareLatest(),
);
...
export const baselineID$ = alternatives$.pipe(
	map(
		(alternatives) =>
			alternatives.find((alternative) => alternative.baseline)?.id ?? -1,
	),
);
...
export const costs$ = costIDs$.pipe(
	switchMap((ids) =>
		liveQuery(() => db.costs.where("id").anyOf(ids).toArray()),
	),
);

export const getEmissionsRateOption = (
	option: EmissionsRateScenario | undefined,
) => {
	switch (option) {
...
const getSccOption = (
	option: SocialCostOfGhgScenario | undefined,
): string | undefined => {
	switch (option) {
...
export namespace Model {
	export namespace Location {
		...
		export const country$ = merge(
			sCountry$,
			location$.pipe(map((p) => p.country)),
		).pipe(distinctUntilChanged());
		sCountry$
			.pipe(withLatestFrom(projectCollection$))
			.subscribe(([country, collection]) =>
				collection.modify({ "location.country": country }),
			);
		...
		export const city$ = merge(sCity$, location$.pipe(map((p) => p.city))).pipe(
			distinctUntilChanged(),
		);
		sCity$
			.pipe(withLatestFrom(projectCollection$))
			.subscribe(([city, collection]) =>
		...
		export const usLocation$ = location$.pipe(
			filter(
				(location): location is USLocation => location.country === Country.USA,
			),
		);
		export const nonUSLocation$ = location$.pipe(
			filter(
				(location): location is NonUSLocation =>
		...
		export const state$ = merge(
			sState$,
			usLocation$.pipe(map((usLocation) => usLocation.state)),
		).pipe(distinctUntilChanged());
		sState$
			.pipe(withLatestFrom(projectCollection$))
			.subscribe(([state, collection]) =>
				collection.modify({ "location.state": state }),
			);
		...
		sStateOrProvince$
			.pipe(withLatestFrom(projectCollection$))
			.subscribe(([stateOrProvince, collection]) =>
				collection.modify((project) => {
	...
	export const [useName, name$] = bind(
		merge(newName$, dbProject$.pipe(map((p) => p.name))).pipe(
			distinctUntilChanged(),
		),
		undefined,
	);
	newName$
		.pipe(withLatestFrom(projectCollection$))
		.subscribe(([name, collection]) => collection.modify({ name }));
	...
	export const analyst$ = state(
		merge(sAnalyst$, dbProject$.pipe(map((p) => p?.analyst))).pipe(
			distinctUntilChanged(),
		),
		undefined,
	);
	...
	export const description$ = state(
		merge(sDescription$, dbProject$.pipe(map((p) => p?.description))).pipe(
			distinctUntilChanged(),
		),
		undefined,
	);
	sDescription$
		.pipe(withLatestFrom(projectCollection$))
		.subscribe(([description, collection]) =>
			collection.modify({ description }),
		);
	...
	export const releaseYearsResponse$ =
		ajax.getJSON<ReleaseYearResponse[]>("/api/release_year");
	...
	export const releaseYear$ = merge(
		sReleaseYear$,
		dbProject$.pipe(map((p) => p.releaseYear)),
	).pipe(distinctUntilChanged());
	sReleaseYear$
		.pipe(withLatestFrom(projectCollection$))
		.subscribe(([releaseYear, collection]) =>
			collection.modify({ releaseYear }),
		);
	...
	export const studyPeriod$ = state(
		merge(sStudyPeriod$, dbProject$.pipe(map((p) => p.studyPeriod))).pipe(
			distinctUntilChanged(),
		),
		25,
	);
	sStudyPeriod$
		...
		.subscribe(([studyPeriod, collection]) =>
			collection.modify({ studyPeriod }),
		);
	...
	export const constructionPeriod$ = state(
		merge(
			sConstructionPeriod$,
			dbProject$.pipe(map((p) => p.constructionPeriod)),
		).pipe(distinctUntilChanged()),
		0,
	);
	sConstructionPeriod$
		.pipe(withLatestFrom(projectCollection$))
		.subscribe(([constructionPeriod, collection]) =>
			collection.modify({ constructionPeriod }),
		);
	...
	export const purpose$ = state(
		merge(sPurpose$, dbProject$.pipe(map((p) => p.purpose))).pipe(
			distinctUntilChanged(),
		),
		Purpose.COST_LEASE,
	);
	...
	export const analysisType$ = state(
		merge(sAnalysisType$, dbProject$.pipe(map((p) => p.analysisType))).pipe(
			distinctUntilChanged(),
			guard(),
		),
		undefined,
	...
	export const dollarMethod$ = state(
		merge(sDollarMethod$, dbProject$.pipe(map((p) => p.dollarMethod))).pipe(
			distinctUntilChanged(),
		),
	...
	sDollarMethod$
		.pipe(withLatestFrom(projectCollection$))
		.subscribe(([dollarMethod, collection]) =>
			collection.modify({ dollarMethod }),
		);
	...
	export const inflationRate$ = state(
		merge(sInflationRate$, dbProject$.pipe(map((p) => p.inflationRate))).pipe(
			distinctUntilChanged(),
		),
		undefined,
	);
	...
	export const nominalDiscountRate$ = state(
		merge(
			...
			dbProject$.pipe(map((p) => p.nominalDiscountRate)),
		).pipe(distinctUntilChanged()),
		undefined,
	);
	...
	export const realDiscountRate$ = state(
		merge(
			sRealDiscountRate$,
			dbProject$.pipe(map((p) => p.realDiscountRate)),
		).pipe(distinctUntilChanged()),
		undefined,
	);
	...
	sInflationRate$
		.pipe(withLatestFrom(projectCollection$))
		.subscribe(([inflationRate, collection]) =>
			collection.modify({ inflationRate }),
		);
		...
		.pipe(
			switchMap((method) => {
				...
				return combineLatest([realDiscountRate$, inflationRate$]).pipe(
					map(([real, inflation]) =>
						calculateNominalDiscountRate(real ?? 0, inflation ?? 0),
					),
				);
			}),
			withLatestFrom(projectCollection$),
		)
		.subscribe(([nominalDiscountRate, collection]) =>
			collection.modify({ nominalDiscountRate }),
		);
		...
		.pipe(
			...
			switchMap((method) => {
				...
				return combineLatest([nominalDiscountRate$, inflationRate$]).pipe(
					map(([nominal, inflation]) =>
						calculateRealDiscountRate(nominal ?? 0, inflation ?? 0),
					),
				);
			}),
			withLatestFrom(projectCollection$),
		)
		.subscribe(([realDiscountRate, collection]) =>
			collection.modify({ realDiscountRate }),
		);
	...
	export const discountingMethod$ = merge(
		sDiscountingMethod$,
		dbProject$.pipe(map((p) => p.discountingMethod)),
	).pipe(distinctUntilChanged());
	sDiscountingMethod$
		.pipe(withLatestFrom(projectCollection$))
		.subscribe(([discountingMethod, collection]) =>
			collection.modify({ discountingMethod }),
		);
	...
	export const emissionsRateScenario$ = state(
		merge(
			...
			dbProject$.pipe(map((p) => p.ghg.emissionsRateScenario)),
		).pipe(distinctUntilChanged()),
	...
	export const sSocialCostOfGhgScenario$ =
		new Subject<SocialCostOfGhgScenario>();
	export const socialCostOfGhgScenario$ = state(
		merge(
			sSocialCostOfGhgScenario$,
			dbProject$.pipe(map((p) => p.ghg.socialCostOfGhgScenario)),
		).pipe(distinctUntilChanged()),
	);
	sSocialCostOfGhgScenario$
		.pipe(withLatestFrom(projectCollection$))
	...
	export const scc$ = combineLatest([
		...
		map((response) => response.response),
		map((dollarsPerMetricTon) =>
			dollarsPerMetricTon.map((value) => value / 1000),
		),
		startWith(undefined),
	);
