 ...
 */
function toRateChangeInfo([changes, releaseYear]: [
	number[],
	number,
]): RateChangeInfo[] {
...
export namespace RecurringModel {
	const recurringOptic = O.optic<Cost>()
		.guard(isRecurringCost)
		.prop("recurring");
	 ...
	 */
	export const [isRecurring] = bind(
		recurring.$.pipe(map((recurring) => !!recurring)),
	);
	...
	export namespace Value {
		...
		sampleMany(setDefaultArray$, [
			Model.studyPeriod.$,
			Model.constructionPeriod.$,
		]).subscribe(([studyPeriod, constructionPeriod]) =>
			rate.set(makeArray((studyPeriod ?? 0) + constructionPeriod, 0)),
		);
