...
export namespace EnergyCostModel {
	...
	export const cost$ = CostModel.cost$.pipe(
		filter((cost): cost is EnergyCost => cost.type === CostTypes.ENERGY),
	);
	...
	export const customerSector$ = merge(
		sSectorChange$,
		cost$.pipe(map((cost) => cost.customerSector)),
	).pipe(guard(), distinctUntilChanged());
	combineLatest([sSectorChange$, CostModel.collection$]).subscribe(
		([customerSector, costCollection]) =>
	...
	export const fuelType$ = merge(
		sFuelTypeChange$,
		cost$.pipe(map((cost) => cost.fuelType)),
	).pipe(distinctUntilChanged());
	combineLatest([sFuelTypeChange$, CostModel.collection$]).subscribe(
		([fuelType, costCollection]) => costCollection.modify({ fuelType }),
	);
	...
	export const escalation$ = state(
		merge(rateChange$, cost$.pipe(map((cost) => cost.escalation))).pipe(
			distinctUntilChanged(),
		),
		0,
	);
	 ...
	 * Use index streams
	 */
	export const [useIndexChange$, useIndexChange] = createSignal<
		number | number[]
	>();
	const newUseIndex$ = useIndexChange$.pipe(shareReplay(1));
	export const useIndex$ = merge(
		newUseIndex$,
		cost$.pipe(map((cost) => cost.useIndex)),
	).pipe(distinctUntilChanged(), shareReplay(1));
	combineLatest([newUseIndex$, CostModel.collection$]).subscribe(
		([newRates, costCollection]) =>
	...
	export const unit$ = merge(
		sUnitChange$,
		cost$.pipe(map((cost) => cost.unit)),
	).pipe(distinctUntilChanged(), shareReplay(1));
	export const [useUnit] = bind(unit$, EnergyUnit.KWH);
	combineLatest([sUnitChange$, CostModel.collection$]).subscribe(
		([unit, costCollection]) => costCollection.modify({ unit }),
	);
	...
	export namespace Location {
		...
		export const customLocation$ = state(
		...
		export const country$ = merge(
			sCountry$,
			location$.pipe(map((location) => location?.country)),
		).pipe(distinctUntilChanged());
		sCountry$
			.pipe(withLatestFrom(CostModel.collection$))
		...
		export const city$ = merge(
			sCity$,
			location$.pipe(map((location) => location?.city)),
		);
		sCity$
			.pipe(withLatestFrom(CostModel.collection$))
			.subscribe(([city, collection]) =>
				collection.modify({ "location.city": city }),
			);
