...
export namespace ResultModel {
	...
	export const result$ = hash$.pipe(
		switchMap((hash) => liveQuery(() => db.results.get(hash))),
	);
	export const [noResult] = bind(
		result$.pipe(map((result) => result === undefined)),
		true,
	);
	export const [hasError] = bind(
		result$.pipe(
			map((result) => result !== undefined && !("measure" in result)),
		),
		false,
	);
	 ...
	 */
	export const [useTimestamp] = bind(
		result$.pipe(map((result) => result?.timestamp)),
		undefined,
	);
	 ...
	 */
	export const [isLoading] = bind(
		merge(loading$, result$.pipe(map(() => false))),
		false,
	);
	...
	export const alternativeNames$ = alternatives$.pipe(
		map(createAlternativeNameMap),
	);

	export const [selectChange$, selectAlternative] = createSignal<number>();
	export const selection$ = merge(
		selectChange$,
		alternatives$.pipe(map((alternatives) => alternatives[0].id ?? 0)),
	);
	...
	export const selectedMeasure$ = combineLatest([measures$, selection$]).pipe(
		map(([measures, selection]) =>
			measures.find((measure) => measure.altId === selection),
		),
		guard(),
		shareReplay(1),
	);
	...
	export const optionalsByTag$ = optionals$.pipe(
		map((optionals) => groupOptionalByTag(optionals)),
	);
	export const [useOptionalsByTag] = bind(optionalsByTag$, new Map());

	export const [discountedCashFlow$, setDiscountedCashFlow] =
	...
	export const categoryOptions$ = optionals$.pipe(
		...
		map(([optionals, selection]) =>
			optionals
				.filter(
					(optional) =>
						optional.altId === selection &&
						!arrayIsZero(optional.totalTagCashflowDiscounted),
				) // filter to use data for current alternative and exclude data of only zeroes
	...
	export const [categorySelectionSignal, setCategorySelection] =
		createSignal<string>();
	...
	export const [useCategorySelection, categorySelection$] = bind(
		merge(
			...
			categoryOptions$.pipe(map((categories) => categories[0].value)),
		),
	);
}
